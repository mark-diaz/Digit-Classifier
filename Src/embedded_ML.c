...
void Feature_Extraction_State_Machine_Translation(float * feature_1,
	...
	*/
	...
	/* Collect User Data on Button clock */
	printf("\nStart Motion Segment with Green LED Blink\n");
	printf("Press User Button to collect next feature:\n");
	User_Button_Wait();
	printf("Perform Feature!\n");
...
void Test_ANN_SM_Translation(ANN *net, int32_t *loc) {
	...
	int32_t random_motion = rand() % 6;

	switch(random_motion){
	case 0:
		printf("\nPlease Perform a 1\n");
		break;
	case 1:
	...
	if(random_motion == *loc){
		printf("Correct Motion Detected!\n");
	}
	else {
		printf("Incorrect Motion Detected!\n Please try again.\n");
	}
